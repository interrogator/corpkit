...
def corpkit_gui():
    ...
    tbut.grid(column = 1, row = 10, sticky = E)
...
# a list of every interrogation
    ...
    def update_corpkit(stver):
        ...
        if extens not in corpath and sys.platform != 'darwin':
            ...
            return
        apppath = corpath.split('.%s' %fext , 1)
        if len(apppath) > 1:
            apppath = apppath[0] + '.%s' % fext
        else:
            if apppath[0].endswith('.py'):
                timestring("Can't replace .py with .app, sorry.")
                return
            else:
                apppath = apppath[0]
